...
public:
    CountUntilClientNode() : Node("count_until_client")
    {
        ...
        RCLCPP_INFO(this->get_logger(), "count_until_client has been started");
    }
    ...
    void send_goal(int target_number, double period)
    {
        ...
        options.result_callback = std::bind(&CountUntilClientNode::get_result_callback, this, _1);
...
private:
    ...
    void get_result_callback(const CountUntilGoalHandle::WrappedResult &result)
    {
        ...
        int reatched_number = result.result->reached_number;
        RCLCPP_INFO(this->get_logger(), "Result: %d", reatched_number);
    }
