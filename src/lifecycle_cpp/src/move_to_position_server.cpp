...
class MoveToPosServerNode : public rclcpp_lifecycle::LifecycleNode
{
public:
    MoveToPosServerNode() : LifecycleNode("move_robot_server")
    {
        robot_position_ = 50;
    ...
    LifecycleCallbackReturn on_configure(const rclcpp_lifecycle::State &previous_state)
    {
        ...
        move_robot_server_ = rclcpp_action::create_server<MoveToPos>(
            this,
...
private:
    rclcpp_action::GoalResponse goal_callback(
        const rclcpp_action::GoalUUID &uuid, std::shared_ptr<const MoveToPos::Goal> goal)
    {
        (void)uuid;
        ...
        // Validate new goal
        if ((goal->target_position < 0) || (goal->target_position > 100) || (goal->velocity <= 0)) {
    ...
    rclcpp_action::CancelResponse cancel_callback(
        const std::shared_ptr<MoveToPosGoalHandle> goal_handle)
    {
        (void)goal_handle;
        ...
        const std::shared_ptr<MoveToPosGoalHandle> goal_handle)
    {
        ...
        int goal_position = goal_handle->get_goal()->target_position;
    ...
    rclcpp_action::Server<MoveToPos>::SharedPtr move_robot_server_;
